...
namespace Pdfsm
{
	...
	class StateBehavior :
		public _s<decltype(EnumValue), EnumValue>,
	...
	protected:
		...
		{
			if (!tt[from][to])
				throw std::runtime_error("pdfsm: invalid jump from " + std::to_string(from) + " to " + std::to_string(to));
		}
	...
	public:
		...
		{
			assert(m != nullptr);
			if (bt[m->stack[m->top]]->BeforeUpdate(ctx))
				return;
